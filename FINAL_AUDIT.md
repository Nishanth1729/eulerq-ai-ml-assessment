...
| Notebook deliverable | PASS | `task1/customer_segmentation.ipynb` is present and was executed end-to-end; the executed notebook and figures are in the submission archive. |
...
| Short written report | PASS | Current authoritative report is `task1/report.md`; it covers decisions, method, profiles, inferences and limitations. The older PDF is stale and intentionally excluded from the clean archive. |

## Task 2 — Offline Text-to-SQL

| Requirement | Status | Evidence |
|---|---|---|
| Use an SLM | PASS | Packaged local T5-Small sequence-to-sequence model/tokenizer under `task2/model/text-to-sql-small`; auxiliary local intent classifier files are `tiny_slm.pt`, `vocab.json`, and `labels.json`. No runtime model download is configured. |
...
| Schema-agnostic behavior | PASS | `task2/schema_agnostic_validation.py`: 9/9 semantic examples across Products and Employees, each a materially different 10-column schema; `task2/exe_validation.py`: the same 9 unrelated-schema examples passed in the packaged exe. |
...
| Clean submission archive | PASS | `eulerq_ai_ml_assessment_submission_final.zip` contains 1,038 files; `ZipFile.testzip()` passed. Archive inspection confirms the executable, model/tokenizer, dataset, notebook and audit are present, with no build tree, virtual environment, cache or unused FLAN weights. |
...
- `task2/build*`, `task2/dist` and local virtual-environment/cache files remain in the working directory. Recursive cleanup was blocked by the workspace approval policy. They are excluded from the clean submission archive; submit the ZIP rather than the raw working folder.
